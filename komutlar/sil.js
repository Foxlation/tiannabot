  ...
  exports.run = function(client, message, args) {
    ...
    if (isNaN(args[0])) {
      var errembed = new Discord.RichEmbed()
        ...
        .addField(
          `Doğru Kullanım:`,
          `${ayarlar.prefix}temizle <temizlenecek mesaj sayısı>`
        );
      return message.channel.send(errembed).then(m => m.delete(5000));
    }
    ...
    message.channel.bulkDelete(args[0]).then(deletedMessages => {
      if (deletedMessages.size < 1)
        return message.reply(
          "Hiç mesaj silemedim! _(**14** günden önceki mesajları silemem!)_"
        );
    });
    message.channel
      .send(`Başarıyla **${args[0]}** Adet Mesaj Silindi!`)
      .then(msg => {
        msg.delete(5000);
      });
  };
