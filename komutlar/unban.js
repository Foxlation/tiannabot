...
exports.run = async(client, message, args) => {
  ...
  if(!args[0]) return message.reply('Kaldırılacak banlı kişinin banını kaldırmak için ID yazmalısın. Eğer toplu ban kaldırmak istiyorsan;\n **-unban toplu** olarak yazmalısın. ')
    if(args[0] === "toplu") {
    ...
    try {
      ...
      client.fetchUser(args[0]).then(x => message.channel.send(new Discord.RichEmbed().setAuthor('Ban başarıyla kaldırıldı!').setTimestamp().setColor("RANDOM").setFooter(message.guild.name, message.guild.iconURL).setDescription(`**Banı Kaldırılan Kullanıcı:** ${x.tag} \n**Banı Kaldıran Yetkili:** ${message.author} | ${message.author.id}`)))
